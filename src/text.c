...
struct Renderable_Text renderable_texts[MAX_STRINGS];
    ...
    float a)
{
    ...
    // VAO not supported in OpenGL ES 2.0 - vertex attributes will be set up during rendering
...
void draw_texts()
{
    ...
    {
        // Set up vertex attributes directly (no VAO support in OpenGL ES 2.0)
        glBindBuffer(GL_ARRAY_BUFFER, renderable_texts[i].points_vbo);
        glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 0, NULL);
        glEnableVertexAttribArray(0);

        glBindBuffer(GL_ARRAY_BUFFER, renderable_texts[i].texcoords_vbo);
