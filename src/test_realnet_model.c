...
static int ensure_texture(GLint w, GLint h){
    if(g.tex && g.tex_w==w && g.tex_h==h) return 1;
...
int init_gl33_window(const char* title, int win_w, int win_h){
    ...
    if(!g.window){ fprintf(stderr,"glfwCreateWindow failed\n"); glfwTerminate(); return 0; }
...
void shutdown_gl33(void){
    ...
    if(g.window){ glfwDestroyWindow(g.window); g.window=NULL; }
    glfwTerminate();
    memset(&g,0,sizeof(g));
}
...
void draw_rgb8_to_window_33(const unsigned char* rgb, int w, int h){
    if(!g.window) return;
    if(glfwWindowShouldClose(g.window)) return;
...
int window_should_close_33(void){
    return g.window && glfwWindowShouldClose(g.window);
}
...
int main(int argc, char* argv[]) {
    ...
    while (!window_should_close_33()) {
        ...
        // 12) Temizlik
        if (sorted_dets) free(sorted_dets);
        if (dets) free_detections(dets, nboxes);
        free_image(sz);
        free_imgdat(imgdat);

        // 13) Çıkış kontrolü
        if (window_should_close_33()) break;
